...
class Trie{
    ...
    addWord(str) {
        ...
        for(let i = 0; i < str.length; i++){
            ...
            if(node.children[character] === undefined){
                node.children[character] = new Vertex(character);
            }
    ...
    getProperSuffixes(word) {
        ...
        for(let index = 1; index < word.length; index++){
            suffixes.push(word.substring(index, word.length));
        }
        suffixes.push("");
        return suffixes;
    }
    ...
    getFailureNode(word) {
        ...
        if(word){
            for(let i = 0; i < word.length; i++){
                ...
                // Note: breaking the loop in the else clause of this statement
                // could result in better performance, but when tested it reduces the number
                // of outputs
            }
        }
        return node;
        ...
        while( i < str.length ){
            ...
            if(root.children[character] !== undefined){
                ...
                if(root === this.root){
                    ...
                    root = root.links.failure;
                }
                continue;
            }
            if(root.isFinal()){
                matchIndices.push([(i - root.word.length), i]);
            }
            if(root.links.outputs.length > 0){
                for(let j = 0; j < root.links.outputs.length; j++){
                    matchIndices.push([(i - root.links.outputs[j].word.length), i]);
                }
            }
        }
        return matchIndices;
...
console.log(trie.getMatches(text).map((indices) => text.substring(indices[0], indices[1])).length)